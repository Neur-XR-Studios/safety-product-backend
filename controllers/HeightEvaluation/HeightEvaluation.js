...
const createOrUpdate = async (req, res) => {
  ...
  if (sessionId === null || !Number.isInteger(sessionId)) {
    return res
      .status(400)
      .json({
        message: "Failed to create or update Learning entry",
        error: "Trainee Session ID must be a non-null integer",
      });
  }
...
const deleteById = async (req, res) => {
  const { sessionId } = req.params;
  try {
    const deletedEvaluation = await WorkAtHeight.findOneAndDelete({
      sessionId,
    });
