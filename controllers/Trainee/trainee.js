...
const index = async (req, res) => {
  ...
  try {
    res
      .status(200)
      .json({ message: "Trainee Data Retrieved Successfully", data: trainee });
  } catch (error) {
  ...
  try {
    ...
    if (sessionIdExist != null) {
      ...
      if (existingTrainee) {
        ...
        const data = {
          traineeID: updatedTrainee._id,
          sessionId: updatedTrainee.sessionId,
          name: updatedTrainee.name,
          phoneNumber: updatedTrainee.phoneNumber,
          company: companyID,
        };
        return res
          .status(200)
          .json({ message: "Trainee Updated Successfully", data });
      }
    }
    let sessionId;
    do {
    ...
    res
      .status(400)
      .json({ message: "Failed to create Trainee", error: error.message });
  }
};
...
const totalSessionTime = async (req, res) => {
  ...
  try {
    ...
    if (startTime && endTime) {
      try {
        totalTimeTaken = dateTimeServiceInstance.calculateTotalTime(
          startTime,
          endTime
        );
      } catch (error) {
    ...
    if (error.code == 11000) {
      res
        .status(400)
        .json({ message: "Session ID already exists.", error: error.message });
    } else {
      res
        .status(400)
        .json({ message: "Failed to Note Session Time", error: error.message });
    }
  }
};
