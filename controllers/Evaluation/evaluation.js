  ...
  try {
    ...
    if (test1.responseTime) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `test1.responseTime is not valid. Please use the format mm:ss`,
        });
      }
    }
    if (test1.totalTime) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `test1.totalTime is not valid. Please use the format mm:ss`,
        });
      }
    }
    if (test1.extinguishmentTime) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `test1.extinguishmentTime is not valid. Please use the format mm:ss`,
        });
      }
    }
    if (test2.totalTime) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `test2.totalTime is not valid. Please use the format mm:ss`,
        });
      }
    }
    if (test2.responseTime) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `test2.responseTime is not valid. Please use the format mm:ss`,
        });
      }
    }
    if (timeTaken) {
      ...
      if (timeValidationResult === 0) {
        return res.status(400).json({
          message: "Failed to create or update Evaluation",
          error: `Time is not valid. Please use the format mm:ss in ${timeTaken}.`,
        });
      }
    }
    ...
    if (existingEvaluation) {
      ...
      if (existingEvaluation.attemptedSimulation !== undefined) {
        existingEvaluation.attemptedSimulation = attemptedSimulation;
      }

      if (existingEvaluation.passedSimulation !== undefined) {
        existingEvaluation.passedSimulation = passedSimulation;
      }
      ...
      return res.status(201).json({
        message: "New evaluation created successfully",
        data: newEvaluation,
      });
    }
  } catch (error) {
    ...
    return res.status(500).json({
      message: "Failed to create/update evaluation",
      error: error.message,
    });
  }
};
...
const getEvaluationBySessionId = async (req, res) => {
  try {
    ...
    if (evaluation) {
      ...
      res
        .status(404)
        .json({ message: "Evaluation not found for the provided session ID" });
    }
  } catch (error) {
    console.error("Error fetching evaluation by session ID:", error);
    res.status(500).json({
      message: "Failed to fetch evaluation by session ID",
      error: error.message,
    });
  }
};
...
const updateEvaluationBySessionId = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Failed to update evaluation by session ID",
      error: error.message,
    });
  }
};
...
const deleteEvaluationBySessionId = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Failed to delete evaluation by session ID",
      error: error.message,
    });
  }
};
